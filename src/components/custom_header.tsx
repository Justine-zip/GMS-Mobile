...
export default function CustomHeader({ header, icon, iconSet, isHome, isCircled, onPress }: HeaderProps) {
    return (
        <View style={{ padding: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#fff' }}>
            ...
            {icon && (isCircled ? <CircleIcon name={icon} onPress={() => console.log('#d3ffe1' + '50')} /> : <IconRenderer iconSet={iconSet} name={icon} size={20} onPress={onPress} />)}
        </View>
    );
}
